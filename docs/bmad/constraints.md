...
- Excluded from the gate: `Program.cs` wiring, AppHost, generated code — via `[ExcludeFromCodeCoverage]` + coverlet filters.
- Repository behavior tested on EF Core SQLite in-memory; Postgres-specific semantics (`SELECT … FOR UPDATE`) are covered only by the k6/Postgres acceptance tier (documented boundary).
- TDD per story: failing tests first, then implementation.
- Build/test gate runs against `CoreBankDemo.Rebuild.slnf` until the rebuild completes.
...
| A7 | Untested Postgres-specific SQL | SQLite-in-memory for repo logic; `FOR UPDATE` semantics delegated to load-test tier |
| A8 | No coverage tooling | Coverlet threshold in `tests/Directory.Build.props`; exclusions per §4 |
