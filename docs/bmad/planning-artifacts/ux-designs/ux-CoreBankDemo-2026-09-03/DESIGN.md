...
- **Feed status is inline** (`{components.feed-status-inline}`) — `foreground-listening` (`text-muted`) on the awaiting row's own elapsed readout and on the Evidence feed header, never a chip and never a seventh chip family in the topology bar (placement and its rationale: `EXPERIENCE.md` Information Architecture). A lost feed is rendered by the row changing state, not by adding a badge: `Outcome unknown` takes `foreground-lost` (`state-neutral`/`○`), the palette's existing "genuinely not known" tone.
...
- **No new color token exists for settlement.** Settled reuses `state-healthy`, a broadcast rejection `state-failed`, and a payment still awaiting its event `state-running` — the same amber that already means `202 Pending`.
