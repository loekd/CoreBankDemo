...
components:
  ...
  status-chip-failed:
    ...
    symbol: '✕'
  status-chip-devproxy:
    foreground-on: '{colors.accent-teal}'
    foreground-off: '{colors.text-muted}'
    symbol-on: '⚡'
    symbol-off: '·'
  ...
  outcome-query:
    ...
    always-enabled: true
  devproxy-toggle:
    on-background: '{colors.accent-teal}'
    on-foreground: '{colors.surface-base}'
    off-background: '{colors.surface-raised}'
    off-foreground: '{colors.text-secondary}'
...
- **Accent Teal (`{colors.accent-teal}`)** — the single restrained chromatic accent inherited from the deck's cockpit language. Used exclusively for the primary action button and the focused-control indicator. Never used for a state badge, never for chrome, never decoratively — this mirrors the "one accent, used sparingly" discipline that other DESIGN.md examples in this project's toolchain use, adapted here to a control surface rather than a writing surface. `surface-base`-on-`accent-teal` (the primary-action-button's own text/fill pairing) is 7.54:1, well clear of AA.
...
- **State colors — `state-healthy`, `state-running`, `state-neutral`, `state-ambiguous`, `state-failed`, `state-failed-on-raised`.** These carry all pass/fail/unknown meaning in the console and are the one place this DESIGN.md is load-bearing rather than decorative:
  ...
  - **DevProxy chip** (`{components.status-chip-devproxy}`) is a sixth, independent chip — not a pass/fail state — always present in the topology bar: `⚡`/accent-teal when on, `·`/text-muted when off, `unavailable` (text-muted, no symbol) when the current topology doesn't support it. See Components, Topology bar.
...
- **Topology bar** (`{components.topology-bar}`) — persistent, one row, top of shell, on `surface-base` like the canvas beneath it. Shows the current AppHost profile (Regular/LoadTests), Owned/Attached badge, the persistent DevProxy chip (`{components.status-chip-devproxy}`), and a horizontal strip of resource status chips. Never scrolls out of view. Refresh cadence, hysteresis, and immediate command-state behavior are defined in `EXPERIENCE.md` Component Patterns.
- **Navigation rail** (`{components.nav-rail}`) — persistent, compact, fixed-width (`{spacing.nav-rail-width}`) column on the left edge of the shell, `surface-raised` with a single hairline-right divider. Hosts one **nav rail item** (`{components.nav-rail-item}`) per workspace — `accent-navy` fill when active, `surface-raised` when inactive — each reachable by a single keypress (`1`–`4`) or click; never nested, never a horizontal tab strip. Below `{spacing.preferred-width}`, shrinks to `{spacing.nav-rail-width-compact}` (icon+shortcut only, labels hidden).
...
- **DevProxy toggle** (`{components.devproxy-toggle}`) — a two-state pill: `on-background`/`on-foreground` (filled `accent-teal`/`surface-base`) when on, `off-background`/`off-foreground` (`surface-raised`/`text-secondary`) when off, `unavailable-foreground` (`text-muted`, no fill) when the current topology doesn't support it. Its on/off state is mirrored live in the topology bar's persistent DevProxy chip (`{components.status-chip-devproxy}`) — the same two colors/symbols, so a presenter who switches workspaces still sees at a glance that faults are being injected.
...
| Show DevProxy on/off as a persistent topology-bar chip across every workspace | Let fault injection stay silently on after leaving Resources |
