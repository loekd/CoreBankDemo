...
- 2026-08-31: Verified the existing implementation against this spec end-to-end (full file/code read, not just the changelog) and closed the previously-admitted gap in slide 42's evidence: the Inbox cue's assertion (`FirstTransactionId == SecondTransactionId`) only ever compared a client-supplied field echoed back verbatim by both calls, which is not durable evidence — it would pass even if the runner's own idempotency-key derivation were broken. Added `corebank.transactions.status` to the compiled endpoint allow-list (`GET /api/transactions/{idempotencyKey}`, the same frozen CoreBankAPI contract endpoint constraints.md documents), plus a new `PathParamRef` scenario-action field (resolves a prior `CaptureAs` value into a parameterized known endpoint's single path segment — additive to `IHttpActionExecutor.SendAsync`, still allow-listed, still no scenario-supplied URL). Wired it as a non-gating `investigateAction` on the slide-42 cue (not into the automatic pass/fail path, to avoid flakiness from the InboxProcessor's async completion timing) so the speaker can pull up the durable Inbox row — CoreBankAPI's own supported inspection endpoint — on demand, satisfying the frozen I/O matrix's "prove one Inbox identity/one execution through supported inspection endpoints." Also fixed the confidence pane to show speaker-facing names ("Payments API", "Dapr pub/sub", "Load harness", ...) instead of raw resource ids, matching the UX Blueprint. Confirmed via full code read (not just tests) that the owned/attach process adapter (fingerprint-verified `TryAttachAsync`, SIGINT-then-kill `StopOwnedAsync`, bounded redacted output), health monitor, allow-listed HTTP executor, browser launcher, three-pane responsive Terminal.Gui shell (compact layout collapses to a single focused pane per the frozen I/O matrix's "Small/no-mouse terminal" row, full keyboard parity, `AppTerminal.Invoke` UI-thread marshaling), doctor/show/rehearse/resume CLI, journal/proof-pack stores, solution/filter/central-package-pin/gitignore wiring, zero-project-reference project graph, and README presenter-workflow docs are all genuinely complete — not just previously claimed. `dotnet test tests/CoreBankDemo.DemoRunner.Tests` is green at 125/125 with 98.65%/92.19% line/branch coverage on the gated Application/presentation-model surface (comfortably over the 90% gate); `dotnet build CoreBankDemo.Rebuild.slnf` is clean; `--doctor` runs live with the updated scenario and exits 0. Still explicitly blocked, unchanged from the prior course correction: binding to Stories 7.1–7.3's live endpoints, a successful five-invariant rehearsal proof pack, and the timed real-terminal dress rehearsal all require 7.1–7.3 to reach `done` (currently `review` in sprint-status.yaml) and a live/interactive terminal session neither of which this sandboxed pass had — so those tasks and the story itself remain open.
- 2026-09-01: Fixed a topology-switch bug found in review: `SelectTopology` now stops any other *owned* topology (`SessionController.cs`) before starting or attaching the requested profile, because the Regular and LoadTest AppHost profiles both bind the `corebank-api` port — the talk's Regular→LoadTest transition (slides 45–52) would otherwise fail to start the second profile with that port already held. An attached (unowned) topology is never stopped, per ADR-015. Covered by two new `SessionControllerTests` cases (stops-owned-on-switch, never-stops-attached). `dotnet test` remains green at 127/127, 98.87%/91.94% line/branch coverage.
