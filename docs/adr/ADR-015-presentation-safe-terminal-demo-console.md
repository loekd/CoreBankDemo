...
`CoreBankDemo.DemoRunner` contains **no project reference** to `CoreBankDemo.CoreBankAPI`, `CoreBankDemo.PaymentsAPI`, `CoreBankDemo.Messaging`, `CoreBankDemo.LoadTestSupport`, or any EF Core `DbContext`. That invariant is unchanged and is the one this ADR actually guards: `dotnet list CoreBankDemo.DemoRunner/CoreBankDemo.DemoRunner.csproj reference` must show zero banking implementation projects, forever.

Two of the original clauses no longer hold and are replaced here. The console is no longer "BCL plus one UI package": alongside the pinned `Terminal.Gui`, it takes a package reference to **`Dapr.Messaging`** (pinned centrally at the same 1.18.x as the rest of the Dapr SDK). And it no longer "never opens a Postgres, Redis, Dapr, or container-engine socket": it opens exactly one Dapr socket — an **outbound gRPC stream to a `daprd` sidecar it spawns and owns itself** — so that a submitted payment can resolve on its own instead of sitting at `202 Pending` until the operator remembers to run an outcome query. It still never opens a Postgres, Redis, or container-engine socket, and never speaks to the broker directly; Redis is reached only by that sidecar, configured from the profile's checked-in `dapr/components*` directory. The sidecar hosts no inbound listener and is started without `--app-port`, because a streaming subscription is outbound-dialled; it is owned, PID-verified, and torn down exactly like the AppHost child process (Stop, Switch, quit, and the topology-disappeared branch).

The boundary this buys is worth stating plainly: the console's sidecar runs under its **own app-id**, so Dapr derives its own Redis consumer group and PaymentsAPI's `payments-api`-scoped subscription keeps receiving every event — the console's copy is a fan-out, never a diversion. The three `transaction-events` CloudEvent contracts are declared inside the console as **local wire records** rather than referenced from `CoreBankDemo.ServiceDefaults`, which is precisely how the package addition avoids becoming a project-graph addition. Nothing is added to any banking service, Dapr component, subscription manifest, or message contract for the console's benefit.

Everything else is as before: all interaction with the banking system happens through stable local HTTP endpoints (PaymentsAPI, CoreBankAPI, LoadTestSupport's REST surface — reusing the same endpoints as `demo-requests.http`/`payment-idempotency-tests.http`, never new ones), a narrowly scoped child-process adapter that starts or attaches to a known Aspire AppHost profile, and now this read-only listener.

### Terminal.Gui as the pinned TUI adapter

Terminal.Gui's stable v2 line is the only UI package, pinned centrally at **2.5.0** in `Directory.Packages.props` (one version for the whole repo, consistent with existing central package management). **Amended (Terminal.Gui v2 instance migration, 2026-09-12):** bumped from 2.4.17. The static `Terminal.Gui.App.Application` class this console originally used is obsolete and slated for removal; the console now owns an instance-based `IApplication` created in `Program.cs` and threaded explicitly to the views and services that need it. See `docs/superpowers/specs/2026-09-12-demorunner-terminalgui-v2-migration-design.md`. The package boundary stays thin: `Terminal/` renders immutable presentation-state view models and emits user intents onto the application's command channel; it contains no process or HTTP logic. This keeps application state, authority rules, and evidence handling unit-testable without a real terminal — Terminal.Gui rendering is exercised only behind a thin, fake-able adapter interface.
...
- **Attach:** if a healthy topology matches the expected resource/service/endpoint fingerprint, the runner offers explicit Attach instead of starting a second instance. Attached processes remain *unowned*: whole-AppHost Stop and Switch are forbidden. A fresh Aspire snapshot the console could **read** separately grants **resource-command authority** for allow-listed `aspire resource <resource> start|stop|restart` operations. Stop and Restart are individually confirmed; Start is not, because it destroys nothing. An unreachable, unparseable or stale snapshot — or one the observation debouncer has not yet confirmed — revokes that authority until a fresh readable snapshot succeeds. **Amended (resource lifecycle, 2026-09-10):** the topology's *shape* no longer bears on that authority. A resource the operator stopped from the console changes the fingerprint without changing the run, and keying authority on the fingerprint made the console refuse to start back up what it had just stopped. Run identity, for both command authority and event attribution, is profile plus run generation. Attach still requires a matching fingerprint, and load-test gating still goes through the full readiness check.
...
- `CoreBankDemo.DemoRunner/CoreBankDemo.DemoRunner.csproj` — standalone net10.0 executable; `Terminal.Gui` and `Dapr.Messaging` package references, both versions resolved centrally, and no project reference of any kind.
...
- `CoreBankDemo.DemoRunner/Infrastructure/` — owned Aspire process lifecycle, supported Aspire state/resource CLI, allow-listed HTTP/LoadTestSupport operations, health monitor, browser opener, bounded/redacted evidence export, and the owned `daprd` sidecar plus the streaming `transaction-events` subscription behind `IOutcomeFeed`.
