...
- `CoreBankDemo.CoreBankAPI/Inbox/TransactionIntakeHandler.cs` -- new `RejectAsync(request, errors, ct)`, modelled on `TransactionCancellationHandler`'s tombstone: store the rejection row and enqueue `transaction.failed` before one `StoreIfNewAsync`; detach the event row when the store is lost or throws.
