...
> **Migrated from:** `docs/bmad/implementation-artifacts/spec-7-4-reusable-terminal-demo-operator-console.md` on 2026-09-10
> **Related:** PR #2; [ADR-015](../../adr/ADR-015-presentation-safe-terminal-demo-console.md); [ADR-018](../../adr/ADR-018-instant-payment-rail.md)
...
- 2026-09-04: Adversarial defect repair reopened the story. Aspire failures now remain Unreachable and gate Start; ownership requires verified exact PIDs; async state/provenance and shutdown are guarded; resource ambiguity/partial mutations require reconciliation; the real Terminal.Gui shell and production adapters have executable coverage; and LoadTestSupport now supplies authoritative inline-settlement counts plus genuine durable FIFO ordering evidence. The ordering assertion exposed and drove fixes to both instant inline paths so they use the shared partition lock and only claim the oldest row.
- 2026-09-04: Final three-layer adversarial review (blind-hunter, edge-case-hunter, verification-gap) of the defect-repair round found ten confirmed issues, all fixed: (1) both instant inline paths were discarding a genuinely-completed result whenever the partition lock was lost *after* the workload finished (indistinguishable from "lock never acquired") — the caller could be told `202 Pending` for a payment/transaction that had actually committed; now a non-null result is always trusted and returned regardless of lock-loss timing. (2) Both paths had no exception handling around the new lock-acquisition call, so a lock-backend failure (e.g. Redis) could propagate as an unhandled exception instead of the established graceful fallback to the background rail. (3) A row driven to terminal `Failed` by the new retry-exhaustion path on CoreBank's side could still be reported as generic `Pending`; it now reflects `Failed`. (4) Added regression tests for all three. (5) The FIFO-ordering-violation detector grouped observations by exact-equal timestamp, silently losing detection power for same-tick rows in the same partition (realistic under real concurrent load) — now tiebreaks by row identity, matching the existing claim-query's own ordering. (6) A load-test evidence endpoint 500'd on a null body instead of returning 400. (7) Two DemoRunner console robustness gaps (uncaught exceptions in `StopAsync` and background AppHost-reconciliation) that could crash or wedge the console instead of degrading gracefully. (8) A defensive dictionary-indexer fix preventing a possible `KeyNotFoundException` in the topology bar. (9) Removed a dead, duplicate reimplementation of the destructive-confirmation-key rule. (10) Documentation: ADR-018's addendum now describes the third (lock-loss-after-completion) case, and three genuinely-unfixed gaps (self-reported inline-settlement evidence with no DB corroboration; `--doctor` CLI can print `Ok` under cross-profile port contention the TUI itself is unaffected by; "Partial" being overloaded for two distinct mutation outcomes) are logged to `deferred-work.md`. Full suite re-verified: 1032 tests, 0 failures, all coverage gates met.
- 2026-09-04: Retrofit — operating the console after the above round exposed two further defects it had missed, both root-caused live against a running AppHost and fixed; this entry documents them since they were fixed ad hoc rather than through a new story. (1) An instant payment submitted with an operator-supplied idempotency key came back as a contract-violation ("malformed or mismatched response") every time. Cause: PaymentsAPI could legitimately answer `HTTP 200` while `Status` was still `Pending` — a real violation of this story's ADR-018 contract that `HttpPaymentGateway.MapOutcome` (keyed on `(HTTP code, status)` pairs) correctly flagged rather than silently accepted. `HttpPaymentGateway.cs:206` now maps on the status word alone, independent of HTTP code, and `DescribeViolation` reports which specific rule was broken (unexpected transaction/idempotency-key echo, an unrecognized status word, or a non-`200`/`202` code) instead of one generic message. The actual backend fix — CoreBankAPI's async transaction-completed/failed event was never recorded against the outbox row, so a deferred instant payment's real outcome had nowhere to be read back from on the next duplicate query — is documented in [ADR-018's committed-outcome-recording addendum](../../adr/ADR-018-instant-payment-rail.md#addendum-committed-outcome-recording-gap-on-the-deferredasync-completion-path-2026-09-04); that is also why Evidence had been showing "no committed outcome yet" forever, even minutes after CoreBank genuinely completed the payment. (2) Ctrl+C and the "Copy detail" button did nothing, silently. Terminal.Gui's built-in clipboard shells out to `xclip`, which exists in this environment but has no `DISPLAY` to reach, and the failure never reached the operator. `Infrastructure/TerminalClipboard.cs` (new) builds the OSC 52 escape sequence the terminal emulator itself understands, with tmux/screen passthrough and an oversize-payload guard; `Infrastructure/Osc52Clipboard.cs` (new) is installed on the Terminal.Gui driver at start-up so Ctrl+C and the button share the same path and every attempt's outcome reaches the status bar (`MainWindow.ShowClipboardResult`). While in the area, the default idempotency key changed from the fixed constant `"demo-key-001"` to a fresh per-session generated key (`MainWindow.NewSessionKey`) — a hardcoded default across every launch could itself mask duplicate/replay bugs by always exercising the same row. DemoRunner test count: 235 (was 229); full repository suite: 1,105, all passing, all coverage gates met.
...
- Known HTTP transport maps standard, instant, malformed, and ambiguous outcomes; outcome mapping keys on the status word alone, and violation messages name the specific rule broken.
  [`HttpPaymentGateway.cs:206`](../../../CoreBankDemo.DemoRunner/Infrastructure/HttpPaymentGateway.cs#L206)

- OSC 52 terminal clipboard: Ctrl+C and Copy detail both work with no display server, since Terminal.Gui's own clipboard silently fails there.
  [`Osc52Clipboard.cs:21`](../../../CoreBankDemo.DemoRunner/Infrastructure/Osc52Clipboard.cs#L21)
