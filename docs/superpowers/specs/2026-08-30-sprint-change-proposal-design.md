...
- Rerun the sprint planning workflow after the edits; let its deterministic script refresh tracking without status downgrades.
...
6. A rerun of the sprint planning workflow passes readiness and refreshes `sprint-status.yaml` without orphaned entries.
...
- **Next route:** rerun the sprint planning workflow to verify readiness and refresh deterministic tracking
