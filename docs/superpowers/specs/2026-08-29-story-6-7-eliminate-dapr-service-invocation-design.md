...
> **Migrated from:** `docs/bmad/implementation-artifacts/spec-6-7-eliminate-dapr-service-invocation.md` on 2026-09-10
> **Related:** PR #2; [ADR-008](../../adr/ADR-008-single-http-corebank-integration.md); [ADR-013](../../adr/ADR-013-checked-in-openapi-build-time-kiota.md)
...
- `CoreBankDemo.LoadTestSupport/McpTools/LoadTestTools.cs`, `CoreBankDemo.LoadTests/README.md`, `.claude/skills/conventions/SKILL.md`, active planning documents, and current README guidance -- replace stale claims that payment commands are forwarded via Dapr; distinguish the Kiota command hop from the Dapr event hop.
- `deferred-work.md` (removed in the 2026-09-10 migration; see git history) -- resolve/remove the ADR-008 AppHost cleanup item only after the implementation and guard are green.
...
- [x] Add a focused architecture guard for all forbidden APIs/routes/headers/flags/client names across production source, live configuration, scripts, and AppHosts; do not scan build output or fail on clearly historical ADR/spec context. [`NoDaprServiceInvocationArchitectureTests.cs`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/NoDaprServiceInvocationArchitectureTests.cs) scans every `CoreBankDemo.*` project plus `tests/`, `scripts/`, `dapr/`, `.config/`, `.devcontainer/`, `k6/`, and live root scripts/config while excluding build output and `docs/`. It covers the pinned SDK invocation APIs/handlers, case-insensitive and nested `UseDapr` configuration, invocation routes/headers, alternate-client naming, non-empty scan proof, and unconditional AppHost CoreBank reference.
- [x] Strengthen composition tests so `ICoreBankApiClient` resolves only to `KiotaCoreBankApiClient`, uses the logical service-discovery client, and has no alternate production registration. `CoreBankClientRegistrationTests.AddCoreBankApiClient_resolves_kiota_backed_client` now resolves the complete `IEnumerable<ICoreBankApiClient>`, requires exactly one registration, and verifies it is `KiotaCoreBankApiClient`; the composed-pipeline test still proves the logical `corebank-api` service-discovery route.
- [x] Run Kiota adapter/forwarder tests for success, non-2xx, malformed response, timeout, exception, trace propagation, and cancellation; prove no outcome falls back to Dapr. `CoreBankApiClientTests` and `HttpForwardOutboxDeliveryStrategyTests` (pre-existing, stories 5.3/5.4) pass unchanged (147/147 in `CoreBankDemo.PaymentsAPI.Tests`); no Dapr client/type appears anywhere in their outcome paths.
...
- [x] Run the focused projects and full rebuild gate with warnings treated according to the repository's existing policy. `dotnet test` green for `CoreBankDemo.PaymentsAPI.Tests` (147/147), `CoreBankDemo.CoreBankAPI.Tests` (113/113), `CoreBankDemo.ServiceDefaults.Tests` (107/108, 1 skipped), and `dotnet test CoreBankDemo.Rebuild.slnf` (all 5 projects green, coverage thresholds met). `dotnet build CoreBankDemo.Rebuild.slnf` produced no Dapr-obsolete-API warning. `git diff --check` reported no whitespace errors.
...
**Commands:**
- `git grep -n -i -E 'UseDapr|DaprCoreBankApiClient|InvokeMethodAsync|InvokeMethodWithResponseAsync|InvokeMethodGrpcAsync|CreateInvokeMethodRequest|CreateInvokeHttpClient|CreateInvocationInvoker|DaprInvokeHandler|InvocationHandler|InvocationInterceptor|/v1\.0/invoke/|dapr-app-id' -- CoreBankDemo.* tests scripts dapr .config .devcontainer k6 demo-requests.http ':(exclude)tests/CoreBankDemo.PaymentsAPI.Tests/NoDaprServiceInvocationArchitectureTests.cs'`
...
**Single command transport**

- Unconditional service reference keeps normal and DevProxy runs on the same Kiota path.
  [`AppHost.cs:134`](../../../CoreBankDemo.AppHost/AppHost.cs#L134)

- DevProxy now changes proxy routing only, never the application transport.
  [`AppHost.cs:136`](../../../CoreBankDemo.AppHost/AppHost.cs#L136)

**Regression enforcement**

- Semantic danger signals cover pinned Dapr invocation APIs, flags, routes, and headers.
  [`NoDaprServiceInvocationArchitectureTests.cs:39`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/NoDaprServiceInvocationArchitectureTests.cs#L39)

- Guard scope covers production, AppHost, live configuration, and executable demo surfaces.
  [`NoDaprServiceInvocationArchitectureTests.cs:120`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/NoDaprServiceInvocationArchitectureTests.cs#L120)

- AppHost source assertion proves the CoreBank reference remains outside DevProxy branching.
  [`NoDaprServiceInvocationArchitectureTests.cs:105`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/NoDaprServiceInvocationArchitectureTests.cs#L105)

- Composition assertion rejects duplicate production CoreBank client registrations.
  [`CoreBankClientRegistrationTests.cs:23`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/CoreBankClientRegistrationTests.cs#L23)

- Composed call proves Kiota still uses the logical service-discovery HTTP pipeline.
  [`CoreBankClientRegistrationTests.cs:80`](../../../tests/CoreBankDemo.PaymentsAPI.Tests/CoreBankClientRegistrationTests.cs#L80)

**Current guidance**

- Load-test guidance distinguishes the Kiota command hop from Dapr pub/sub.
  [`README.md:67`](../../../CoreBankDemo.LoadTests/README.md#L67)

- MCP output now describes the actual forwarding transport.
  [`LoadTestTools.cs:406`](../../../CoreBankDemo.LoadTestSupport/McpTools/LoadTestTools.cs#L406)
