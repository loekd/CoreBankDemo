...
/// <summary>
/// Story 3.4: <c>ResolveOtlpEndpoint</c>'s full <c>OTLP_ENDPOINT</c>
...
/// <c>OTLP_ENDPOINT</c> via an in-memory configuration source added
...
{
    private static WebApplicationBuilder CreateBuilder(string? otlpEndpoint)
    {
        var builder = WebApplication.CreateSlimBuilder();
        // Explicitly set (or explicitly null out) the key so this test is
        // deterministic regardless of any ambient OTLP_ENDPOINT the
    ...
    [Theory]
    [InlineData("http://lgtm:4317")]
    [InlineData("https://lgtm-collector:4318")]
    ...
    {
        var builder = CreateBuilder("tcp://lgtm:4317");

        var result = builder.ResolveOtlpEndpoint();

        result.Should().Be(new Uri("http://lgtm:4317"));
    }

    [Fact]
    public void Tcp_scheme_with_a_non_default_explicit_port_keeps_that_exact_port()
    {
        var builder = CreateBuilder("tcp://lgtm:9411");

        var result = builder.ResolveOtlpEndpoint();

        result.Should().Be(new Uri("http://lgtm:9411"));
    }

    [Fact]
    public void Tcp_scheme_without_an_explicit_port_defaults_the_port_to_4317()
    {
        var builder = CreateBuilder("tcp://lgtm");
    ...
    {
        var builder = CreateBuilder("lgtm:4317");
