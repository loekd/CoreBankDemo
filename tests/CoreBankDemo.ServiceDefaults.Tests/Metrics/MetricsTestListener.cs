...
namespace CoreBankDemo.ServiceDefaults.Tests.Metrics;
...
/// tests (spec-6-5): scoped to one specific <see cref="BusinessMetrics"/>
/// instance's own <see cref="Meter"/> (by reference, via the internal
/// <c>BusinessMetrics.Meter</c> accessor) rather than <see cref="BusinessMetrics.MeterName"/>
/// alone. Multiple <see cref="BusinessMetrics"/> instances share the same
/// meter name — every composition root creates its own, and so does every
/// test — so name-only filtering would leak measurements across
/// concurrently-running tests (xUnit runs different test classes in
/// parallel by default) and cause flaky "contains exactly one measurement"
/// assertions.
/// </summary>
