...
public class DeadOptionMembersTests
{
    ...
    /// has no consumer inside Messaging today").
    /// <para>
    /// Story 4.7 supplied the real rebuilt
    /// <c>CoreBankDemo.CoreBankAPI/Outbox/MessagingOutboxProcessor.cs</c>
    /// consumer for the messaging-outbox partition, lock, and polling members.
    /// <c>PubSubName</c>/<c>TopicName</c> point at
    /// <c>DaprEventPublisher.cs</c>, their real reader since story 3.3.
    /// </para>
    /// <para>
    /// Updated by story 5.1: PaymentsAPI's legacy inbox/outbox processors were
    /// demolished. <c>OutboxProcessingOptions.PartitionCount</c> is now read
    /// by <c>PaymentStorageHandler</c>; lock expiry and polling are intentionally
    /// not consumed until story 5.4 adds the forwarding processor, so those two
    /// entries point at the kernel outbox base as explicit temporary
    /// placeholders subject to this test's documented path-only limitation.
    /// The shared inbox options already have a real reader in CoreBankAPI.
    /// </para>
    /// </summary>
