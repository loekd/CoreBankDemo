...
/// <summary>
/// <see cref="InboxProcessorOptions.PollingInterval"/>'s fail-fast guard
/// (mirrors story 2.4's <c>OutboxProcessorOptionsTests</c> patch): a
/// non-positive interval must never reach <see cref="InboxProcessorBase{TMessage}"/>'s
/// poll loop, where <see cref="Task.Delay(TimeSpan, CancellationToken)"/>
/// would throw <see cref="ArgumentOutOfRangeException"/> and crash the whole
...
/// </summary>
public class InboxProcessorOptionsTests
{
    [Fact]
    ...
    {
        var act = () => new InboxProcessorOptions { PollingInterval = invalid };
