...
/// </summary>
public class DemoAccountSeederTests : SqliteCoreBankApiTestBase
{
    [Fact]
    ...
    /// two real <see cref="DemoAccountSeeder"/>s concurrently against the same
    /// shared-cache SQLite database; the loser's unique-PK violation on
    /// <c>AccountNumber</c> must be swallowed, not propagated.
    /// </summary>
    [Fact]
