...
public class PaymentsDbContextTests
{
    ...
    [Fact]
    public async Task Inbox_composite_identity_rejects_duplicate_and_allows_different_account()
    {
        await using var store = new SqlitePaymentsStore();
        await using var first = store.CreateContext();
        ...
        await using var differentAccount = store.CreateContext();
        differentAccount.InboxMessages.Add(PaymentsApiTestData.Inbox(accountNumber: "NL20INGB0001234567"));
        var differentAct = () => differentAccount.SaveChangesAsync(TestContext.Current.CancellationToken);
        await differentAct.Should().NotThrowAsync();
    }
