...
public class TransactionEventIntakeWiringTests : IDisposable
{
    ...
    /// database provider so no live Postgres is required and removing the
    /// unrelated outbox hosted service so the test never contacts Redis.
    /// </summary>
    private sealed class PaymentsApiFactory(string connectionString) : WebApplicationFactory<Program>
    {
