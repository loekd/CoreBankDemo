...
internal sealed class SqlitePaymentsStore : IAsyncDisposable
{
    ...
    public (PaymentsDbContext First, PaymentsDbContext Second) CreateSynchronizedContexts()
    {
        ...
        var options = new DbContextOptionsBuilder<PaymentsDbContext>()
            ...
            .Options;
        return (
            new SynchronizedPaymentsDbContext(options, coordinator, saveFirst: true),
            new SynchronizedPaymentsDbContext(options, coordinator, saveFirst: false));
    }
    ...
    private sealed class SaveCoordinator
    {
        ...
        private readonly TaskCompletionSource _firstSaved =
            new(TaskCreationOptions.RunContinuationsAsynchronously);
        private int _readyCount;

        public Task ArriveAsync()
        {
            if (Interlocked.Increment(ref _readyCount) == 2)
            {
                _bothReady.TrySetResult();
            }

            return _bothReady.Task;
        }

        public Task WaitForFirstSaveAsync() => _firstSaved.Task;

        public void CompleteFirstSave() => _firstSaved.TrySetResult();
    }

    private sealed class SynchronizedPaymentsDbContext(
        DbContextOptions<PaymentsDbContext> options,
        SaveCoordinator coordinator,
        bool saveFirst) : PaymentsDbContext(options)
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!saveFirst)
            {
                await coordinator.WaitForFirstSaveAsync().WaitAsync(cancellationToken);
