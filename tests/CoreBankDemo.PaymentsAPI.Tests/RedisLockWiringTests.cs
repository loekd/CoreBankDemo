...
/// <summary>
/// Story 6.2 verification gap: <c>AddServiceDefaultsTests</c> (in
/// CoreBankDemo.ServiceDefaults.Tests) only proves <c>AddServiceDefaults</c>'s
/// own DI-selection logic against a manually registered
/// <c>IConnectionMultiplexer</c> mock — it never exercises Program.cs's
/// actual <c>builder.AddRedisClient("redis")</c> call. A resource-name drift
/// between that call and AppHost.cs's <c>builder.AddRedis("redis", ...)</c>
/// would silently fall back <see cref="IDistributedLockService"/> to
/// <c>NoOpDistributedLockService</c> (a total Inbox/Outbox processing outage)
/// without failing any test or the coverage gate, since Program.cs is
/// excluded from coverage. This test drives the real Aspire Redis client
/// registration — with <c>abortConnect=false</c> so no live Redis is
/// required — through the same call sequence Program.cs uses, under the same
/// "redis" resource name.
/// </summary>
